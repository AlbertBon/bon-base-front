...
export default {
	...
	methods: {
      ...
      // fmData.append(, data2blob(createImgUrl, mime));
      // 添加其他参数
      ...
      that.uploadRequest(url,fmData).then(res => {
        if(res.data.code == '00') {
          if (that.value) {
            that.loading = 2;
            that.$emit('crop-upload-success', res.data.data, field, ki);
          }
        }else{
          that.loading = 3;
          that.hasError = true;
          that.errorMsg = lang.fail;
          that.$emit('crop-upload-fail', res.data, field, ki);
        }
      })
		}
	},
